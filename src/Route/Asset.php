 ...
 * Static File Server
 *
 * The document root is public/, which holds only the front controller. Static
 * files stay in the project root (assets/, template/, uploads/) and the shipped
 * .htaccess and nginx.conf both send every request to public/index.php, so a
 * request path is resolved here against APP_PATH, not against the document
 * root. This class is therefore the only gatekeeper for what leaves the disk,
 * and it is the whole HTTP layer for everything that may.
 *
