...
Every key lives in a scope, stored as `$_SESSION[<SCOPE>][$key]`, so unrelated parts of an application can't overwrite each other's keys. The static methods above all work on the `APP` scope. `Session::scope()` returns a `Laika\Engine\Session\Scope` with the same methods for any other scope:
