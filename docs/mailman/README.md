...
```php
use Laika\Engine\Mailman\Pipeline\Pipeline;
use Laika\Engine\Mailman\Pipeline\ScanResult;
use Laika\Engine\Mailman\Pipeline\Source\ImapSource;
...
Everything throws a `Laika\Engine\Mailman\Exceptions\MailmanException` or one of three subclasses, so there is one hierarchy to catch — PHPMailer's own exception is translated rather than leaking through.
