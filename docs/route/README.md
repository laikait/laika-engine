...
Url::globalPipeline([\Laika\Engine\Shield\Pipeline\ShieldPipeline::class]);  // every matched route
```
...
`index.php`, so `Laika\Engine\Route\Asset` is the only thing deciding what may leave the disk.
