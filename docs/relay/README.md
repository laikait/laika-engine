...
| Class | Role |
|---|---|
| `Laika\Engine\Relay\RelayRegistry` | The container: holds bindings, auto-wires constructors, caches instances |
| `Laika\Engine\Relay\Relay` | Base class for **relays**: static classes that forward every call to a bound service |
| `Laika\Engine\Relay\RelayProvider` | Base class for **providers**, which bind services into the registry |
...
// Closure factory
$registry->singleton('mailer', fn (RelayRegistry $r) => new \Laika\Engine\Mailman\Mailer(config('mail')));
...
Everything throws `Laika\Engine\Relay\Exceptions\RelayException`:
...
| A provider doesn't extend `RelayProvider` | `[ClassName] must extend Laika\Engine\Relay\RelayProvider.` |
