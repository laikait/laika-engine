...
namespace Laika\Engine\Session\Tests\Handler;
...
use Laika\Engine\Session\Handler\ModelHandler;

/**
 * Exercises the laika-model driver against a real database.
 *
 * laika-model resolves connections by name through Laika\Engine\Model\Connection, so
...
class ModelHandlerTest extends TestCase
{
    ...
    {
        if (!class_exists(\Laika\Engine\Model\Model::class)) {
